...
i_init:
            ...
            mov.w   #2000h, R4  ; first register of register/time sequence memory location
            NOP
...
i_end_data_send:
            ...
            mov.w       #1, R10
            jmp init                 ; after this, jump to 'read' program
i_wait3:
...
;            Read Current Data
; --------------------------------------------------------------------------
; -------------- initialize Read Program  ------------ 
init: 
            mov.w   #2004h, R4    ; register of first memory location we will read slave memory address from
            mov.w   #0, R12  ; use this register to track which RTC register read

; -------------- complete Read Program initialization ------------ 
; ------- start -----------
; ------------- main wait -----------
r_main:
            NOP
...
End_data:
            ...
            jmp r_main
; ----------- end end_send process --------
; ----------- send_sl_mem_addr ----------
Send_sl_mem_addr: 
            mov.b   @R4+, R7
            jmp Send_data_start
; ----------- end send_sl_mem_addr ---------
;------------------------------------------------------------------------------
;           Read data
;------------------------------------------------------------------------------
;---------- start_read --------------
